...
# plt.figure()
# time = np.linspace(0, 27, 28)
# for t in range(0, simulation_time):
#     if t == 0:
#         plt.plot(predicted_states_dynamic[t][:, 4], '--.b', label='NLP-predicted y-velocity')
#     else:
#         time_1 = np.linspace(t, 27, 28 - t)
#         time_1 = time_1.tolist()
#         predicted_states_dynamic[t][t, 4] = arr_1[t, 4]
#         plt.plot(time_1, predicted_states_dynamic[t][t:28, 4], '--.b')
# plt.plot(time, arr_1[:, 4], '-*r', label="Close-loop simulated y-velocity")
# plt.xlabel('$Time$')
